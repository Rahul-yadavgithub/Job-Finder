...
export function CompanyTimeline({ companyId }: Props) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await commApi.get(`/companies/${companyId}/timeline`);
  ...
  const commEvents = timeline.filter(e => e.performed_by_layer === 'comm' && e.event_type !== 'note_added');
  const headEvents = timeline.filter(e => e.performed_by_layer === 'admin' && e.event_type !== 'note_added');
  // Both Comm and Admin note additions can show up here for timeline coherence
  const customEvents = timeline.filter(e => e.event_type === 'note_added');
        ...
        {events.map((event) => {
          ...
          return (
            <div key={event.id} className="relative pl-6">
              <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-2 border-white bg-blue-500 shadow-sm z-10" />
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      <span className="font-semibold text-gray-700">
                        ...
                        {event.users?.branches?.name && <span className="text-blue-600 ml-1">({event.users.branches.name})</span>}
                      </span>
                    </div>
                  </div>
  ...
  return (
    <div className="max-w-3xl mx-auto py-2 w-full">
      ...
      <SectionAccordion id="custom" title="Custom Activities">
        <div className="mb-6">
          <ActivityForm companyId={companyId} onSuccess={fetchData} />
        </div>
