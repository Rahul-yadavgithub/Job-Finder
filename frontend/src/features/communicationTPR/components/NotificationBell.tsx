...
export function NotificationBell() {
  ...
  const fetchUnreadCount = async () => {
    try {
      const res = await commApi.get('/notifications/unread-count');
  ...
  const fetchNotifications = async () => {
    try {
      const res = await commApi.get('/notifications');
      if (res.data.success) {
  ...
  const handleMarkAllRead = async () => {
    try {
      await commApi.patch('/notifications/read-all');
  ...
  const handleNotificationClick = async (notif: Notification) => {
    if (!notif.is_read) {
      try {
        await commApi.patch('/notifications/read', { notificationIds: [notif.id] });
