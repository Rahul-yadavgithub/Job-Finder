...
export function ManageProfileModal({ isOpen, onClose }: ManageProfileModalProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/auth/profile`, { profilePhotoUrl: data.secure_url }, { withCredentials: true });
  ...
  const handleRemovePhoto = async (e: React.MouseEvent) => {
    ...
    try {
      setIsUploading(true);
      await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/auth/profile`, { profilePhotoUrl: null }, { withCredentials: true });
      if (refreshUser) await refreshUser();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-[95vw] md:max-w-2xl overflow-hidden relative max-h-[90vh] flex flex-col">
